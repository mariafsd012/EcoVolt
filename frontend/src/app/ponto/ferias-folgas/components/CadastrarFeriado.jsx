"use client";

import { Palmtree, ChevronDown } from "lucide-react";
import { Heebo } from "next/font/google";

const heebo = Heebo({
  subsets: ["latin"],
  weight: ["400", "500", "600", "700"],
});

const campoStyle = {
  height: "56px",
  padding: "0 20px",
  borderRadius: "12px",
  border: "none",
  outline: "none",
  fontSize: "15px",
  fontWeight: 500,
  color: "#374f30",
  backgroundColor: "#ffffff",
  width: "100%",
};

export default function CadastrarFeriado({
  feriado = { nome: "", equipe: "", tipo: "", dataInicio: "", dataFim: "" },
  equipes = [],
  tiposFeriado = [
    { value: "nacional", label: "Nacional" },
    { value: "estadual", label: "Estadual" },
    { value: "municipal", label: "Municipal" },
    { value: "ponte", label: "Ponto facultativo" },
  ],
  onChangeCampo,
  onCadastrar,
  onVerTodos,
  isSubmitting = false,
}) {
  return (
    <section
      style={{ backgroundColor: "#a9c293", borderRadius: "16px", padding: "32px" }}
      className={heebo.className}
    >
      {/* HEADER */}
      <div
        style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "28px" }}
      >
        <Palmtree size={20} className="text-white" />
        <h2 className="text-white font-semibold" style={{ fontSize: "20px" }}>
          Cadastrar feriado
        </h2>
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        {/* INPUT NOME DO FERIADO */}
        <input
          type="text"
          placeholder="Nome do feriado"
          value={feriado.nome}
          onChange={(e) => onChangeCampo("nome", e.target.value)}
          style={{ ...campoStyle, color: feriado.nome ? "#374f30" : "#9aa893" }}
        />

        <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" }}>
          {/* SELECT EQUIPE */}
          <div style={{ position: "relative", flex: "1 1 200px" }}>
            <select
              value={feriado.equipe}
              onChange={(e) => onChangeCampo("equipe", e.target.value)}
              style={{ ...campoStyle, paddingRight: "36px", appearance: "none" }}
            >
              <option value="">Equipe</option>
              {equipes.map((equipe) => (
                <option key={equipe.value} value={equipe.value}>
                  {equipe.label}
                </option>
              ))}
            </select>
            <ChevronDown
              size={16}
              className="text-[#8a9a85] pointer-events-none"
              style={{ position: "absolute", right: "16px", top: "50%", transform: "translateY(-50%)" }}
            />
          </div>

          {/* SELECT TIPO DE FERIADO */}
          <div style={{ position: "relative", flex: "1 1 200px" }}>
            <select
              value={feriado.tipo}
              onChange={(e) => onChangeCampo("tipo", e.target.value)}
              style={{ ...campoStyle, paddingRight: "36px", appearance: "none" }}
            >
              <option value="">Tipo de feriado</option>
              {tiposFeriado.map((tipo) => (
                <option key={tipo.value} value={tipo.value}>
                  {tipo.label}
                </option>
              ))}
            </select>
            <ChevronDown
              size={16}
              className="text-[#8a9a85] pointer-events-none"
              style={{ position: "absolute", right: "16px", top: "50%", transform: "translateY(-50%)" }}
            />
          </div>

          {/* DATA DE INÍCIO */}
          <input
            type="date"
            value={feriado.dataInicio}
            onChange={(e) => onChangeCampo("dataInicio", e.target.value)}
            style={{ ...campoStyle, flex: "1 1 160px" }}
          />

          {/* DATA DE FIM */}
          <input
            type="date"
            value={feriado.dataFim}
            onChange={(e) => onChangeCampo("dataFim", e.target.value)}
            style={{ ...campoStyle, flex: "1 1 160px" }}
          />
        </div>

        {/* BOTÃO CADASTRAR */}
        <div style={{ display: "flex", justifyContent: "center", marginTop: "12px" }}>
          <button
            type="button"
            onClick={onCadastrar}
            disabled={isSubmitting}
            style={{
              backgroundColor: "#3a6b35",
              height: "48px",
              padding: "0 64px",
              borderRadius: "12px",
              color: "#ffffff",
              fontSize: "15px",
              fontWeight: 600,
              opacity: isSubmitting ? 0.6 : 1,
              border: "none",
            }}
            className="hover:opacity-90 transition-opacity"
          >
            {isSubmitting ? "Cadastrando..." : "Cadastrar"}
          </button>
        </div>

        {/* LINK VISUALIZAR TODOS */}
        <button
          type="button"
          onClick={onVerTodos}
          style={{ fontSize: "13px", color: "#ffffff", textDecoration: "underline", textAlign: "center" }}
          className="hover:opacity-80 transition-opacity"
        >
          Visualizar todos os feriados
        </button>
      </div>
    </section>
  );
}